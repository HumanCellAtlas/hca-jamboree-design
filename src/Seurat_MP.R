...
setwd("shared_scratch/group 5/task6/")
...
pdf("pbmc_0.5_tsne_types.pdf")
data.plot = as.data.frame(cbind(pbmc@dr$tsne@cell.embeddings, as.charater(new_meta)))
colnames(data.plot)[3] = "Type"
ggplot(data.plot, aes(x = tSNE_1, y = tSNE_2)) + 
  geom_point(aes(colour = Type),  size = 1, alpha = 0.8) + 
  scale_color_hue(l = 65) + 
  scale_size(range = c(1, 1)) +
  theme_bw() +
  theme(legend.title=element_blank(),
